...
SELECT DISTINCT apellido1 'Primer apellido', apellido2 'Segundo Apellido', nombre 'Nombre'
FROM persona p
WHERE tipo = 'alumno'
...
FROM  persona p
WHERE tipo = 'alumno' 
	AND p.fecha_nacimiento LIKE '%1999%';
...
-- 6. Retorna un llistat dels professors/es juntament amb el nom del departament al qual estan vinculats. El llistat ha de retornar quatre columnes, primer cog0nom, segon cognom, nom i nom del departament. 
-- El resultat estarà ordenat alfabèticament de menor a major pels cognoms i el nom.
SELECT  pa.apellido1 'Apellido 1', pa.apellido2 'Apellido 2', pa.nombre 'Nombre', d.nombre 'Departamento'
FROM  profesor p
INNER JOIN persona pa 
	ON pa.id = p.id_profesor
INNER JOIN departamento d 
	ON d.id = p.id_departamento
ORDER BY pa.apellido1 ASC, pa.apellido2 ASC, pa.nombre ASC;

-- 7. Retorna un llistat amb el nom de les assignatures, any d'inici i any de fi del curs escolar de l'alumne/a amb NIF 26902806M.
SELECT  a.nombre 'Asignatura', c.anyo_inicio 'Inicio curso escolar', c.anyo_fin 'Fin curso escolar'
...
SELECT p.nombre 'Nombre', p.apellido1 'Primer Apellido', d.nombre 'Departamento'
FROM  persona p
...
SELECT d.*, p.id_profesor 'Profesor asociado'
FROM  departamento d
...
SELECT a.nombre 'Asignatura', pe.nombre 'Nombre', pe.apellido1 'Apellido'
FROM  asignatura a
RIGHT JOIN  profesor pr
	ON a.id_profesor = pr.id_profesor
RIGHT JOIN  persona pe
	ON pr.id_profesor = pe.id
WHERE a.id_profesor IS NULL AND pe.tipo = 'profesor';
...
SELECT COUNT(nombre) 'Número total de alumnos'
FROM persona p
WHERE tipo = 'alumno';

-- 2. Calcula quants alumnes van néixer en 1999.
SELECT COUNT(nombre) 'Número total de alumnos 1999'
FROM persona p
WHERE tipo = 'alumno'
	AND p.fecha_nacimiento LIKE '%1999%';

-- 3. Calcula quants professors/es hi ha en cada departament. El resultat només ha de mostrar dues columnes, una amb el nom del departament i una altra amb el nombre de professors/es 
-- que hi ha en aquest departament. El resultat només ha d'incloure els departaments que tenen professors/es associats i haurà d'estar ordenat de major a menor pel nombre de professors/es.
SELECT nombre 'Departamento', COUNT(p.id_departamento) 'Número de profesores'
FROM departamento d
INNER JOIN profesor p
	ON d.id = p.id_departamento
GROUP BY d.nombre
...
SELECT g.nombre 'Grado', COUNT(a.id_grado) 'Asignaturas' 
FROM grado g
LEFT JOIN asignatura a
	ON g.id = a.id_grado
GROUP BY g.nombre
...
SELECT g.nombre 'Grado', a.tipo 'Tipo de Asignatura', SUM(a.creditos) 'Suma de créditos'
FROM grado g
LEFT JOIN asignatura a
	ON g.id = a.id_grado
GROUP BY g.nombre, a.tipo;

-- 8. Retorna un llistat que mostri quants alumnes s'han matriculat d'alguna assignatura en cadascun dels cursos escolars. El resultat haurà de mostrar dues columnes, una columna amb l'any 
-- d'inici del curs escolar i una altra amb el nombre d'alumnes matriculats. 
SELECT ce.anyo_inicio 'Inicio curso escolar', COUNT(am.id_curso_escolar) 'Alumnos'
...
SELECT pe.id, pe.nombre 'Profesor', pe.apellido1 'Apellido 1', pe.apellido2 'Apellido 2', COUNT(a.id_profesor) 'Número de asignaturas'
FROM asignatura a
